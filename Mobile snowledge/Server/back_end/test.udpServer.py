...
class TestUdpServer(unittest.TestCase):
    ...
    def test_server_receive_LOCATION_message(self):
        ...
        self.cur.execute(f"SELECT * FROM data WHERE dev_id='{devId}' AND timestamp='{secondsSinceEpoch}'")
    ...
    def test_server_receive_HELP_message(self):
        ...
        #send help request
        messagetype = 'HELP'
        message = f"{messagetype}:{secondsSinceEpoch}:{devId}:{gpsCoord}:'Varusteongelma'"
        self.server.udp.sendto(bytes(message, "utf-8"), ('::1',50943))
        time.sleep(1)
        ...
        self.assertGreater(self.connection.total_changes, 0)
        self.cur.execute(f"SELECT * FROM help WHERE dev_id='{devId}' AND timestamp='{secondsSinceEpoch}'")

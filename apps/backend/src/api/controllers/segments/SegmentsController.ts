...
export class SegmentsController {
  ...
  getAllSegments = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    const segments = await this.segmentsService.getAllSegments();
    ApiResponseHandler.success(res, segments);
  });

  getSegmentUpdates = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    const { id } = req.params;
    const updates = await this.segmentsService.getSegmentUpdates(id);
    ApiResponseHandler.success(res, updates);
  });

  getAllUpdates = asyncHandler(async (req: Request, res: Response): Promise<void> => {

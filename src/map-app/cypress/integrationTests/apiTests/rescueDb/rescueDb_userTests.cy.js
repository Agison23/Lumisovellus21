...
describe("Rescue database API test", () => {
    ...
    const headersAdmin = {
        ...
        "Authorization": `admin:${crypto.createHash("sha256").update(Buffer.from(adminPassword, "utf8")).digest("hex")}`
    };
    const headersOperator = {
        "Content-Type": "application/json",
        "Authorization": `operator:${crypto.createHash("sha256").update(Buffer.from(operatorPassword, "utf8")).digest("hex")}`
    };
        ...
        describe("Test with wrong authorization", ()=>{
            ...
            it(`GET ${getUsersPath} with right Auth format, return 401 Unauthorized`, ()=>{
                cy.request({
                    ...
                    expect(response.body).to.be.an("object").to.have.property("message").to.equal("Unauthorized");
                });
            });
        });

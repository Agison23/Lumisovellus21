    ...
    describe("Test invalid request params/body",()=>{
      ...
      it(`POST ${userPath} should return error object`, () => {
        cy.request({
          ...
          expect(emailError).to.have.property("msg").to.equal("Ei toimiva shäköposti");
          ...
          expect(firstNameError).to.have.property("msg").to.equal("Puuttuva etunimi");
          ...
          expect(lastNameError).to.have.property("msg").to.equal("Puuttuva sukunimi");
          ...
          expect(passwordError).to.have.property("msg").to.equal("Salasanan oltava vähintään 7 merkkiä");
    ...
    describe("Test normal functionalities", ()=>{
      ...
      it(`PUT ${userPath} should return update information`, () => {
        cy.request({
          ...
          .then((response)=>{
            ...
            const passwordMatch = bcrypt.compareSync(cypressTestUser.Salasana, cypressGetUser.Salasana);
            expect(passwordMatch).to.be.true;
            expect(cypressGetUser).to.have.property("ID");
            expect(cypressGetUser).to.have.property("Etunimi").to.equal(cypressTestUser.Etunimi);
            expect(cypressGetUser).to.have.property("Sukunimi").to.equal(cypressTestUser.Sukunimi);
            expect(cypressGetUser).to.have.property("Sähköposti").to.equal(cypressTestUser.Sähköposti);
            expect(cypressGetUser).to.have.property("Rooli").to.equal(cypressTestUser.Rooli);
      ...
      it(`DELETE ${userPath} should return delete information`, () => {
        cy.request({
          ...
          .then((response)=>{
            ...
            expect(cypressCurrentUser).to.have.property("Etunimi").to.equal(cypressPutUser.Etunimi);
            expect(cypressCurrentUser).to.have.property("Sukunimi").to.equal(cypressPutUser.Sukunimi);
            expect(cypressCurrentUser).to.have.property("Sähköposti").to.equal(cypressPutUser.Sähköposti);
            // because the credential should not change
            expect(cypressCurrentUser).to.have.property("Rooli").to.equal(cypressTestUser.Rooli);
      ...
      it(`POST ${loginUserPath} should login user, return user info`, ()=>{
        cy.request({
          ...
          expect(decodedJWT).to.have.property("Sahkoposti").to.equal("admin@gmail.com");

          expect(response.body).to.have.property("user").to.be.an("object");
          expect(response.body.user).to.have.property("Etunimi").to.equal("admin");
          expect(response.body.user).to.have.property("Sukunimi").to.equal("admin");
          expect(response.body.user).to.have.property("Rooli").to.equal("admin");
          expect(response.body.user).to.have.property("Sähköposti").to.equal("admin@gmail.com");
        });
      });

    ...
    describe.skip("Test invalid request params/body",()=>{
        ...
        it.skip(`POST ${userOtherObservationPath}/10000 should return database object with no change`,()=>{
            cy.request({
                ...
                expect(response.body).to.have.property('info').to.equal("Rows matched: 0  Changed: 0  Warnings: 0")
    ...
    describe("Test normal functionalities", ()=>{
        ...
        it(`POST ${userReviewPath}/1 should return object with LAST_INSERT_ID()`,()=>{
            ...
            cy.request({
                ...
                expect(response.body[0]).to.be.an('object').to.have.property('LAST_INSERT_ID()').to.be.a('number')
            })
            ...
            cy.request({
                ...
                expect(aika instanceof Date).to.be.true;
                expect(insertedReview).to.have.property("Lisätiedot").to.equal(body.Lisätiedot)
                expect(insertedReview).to.have.property("Lumilaatu").to.equal(body.Lumilaatu)
                expect(insertedReview).to.have.property("Kommentti").to.equal(body.Kommentti)
                expect(insertedReview).to.have.property("Segmentti").to.equal(parseInt(body.Segmentti))
            })
        })

        it(`POST ${userOtherObservationPath}/1 should return database object`,()=>{
            const randomString = Cypress._.random(0, 1e9).toString(36).substr(0, 10);
            const body = {
            ...
            cy.request({
                ...
                expect(response.body).to.have.property('insertId').to.equal(0)
                expect(response.body).to.have.property('info').to.equal("Rows matched: 1  Changed: 1  Warnings: 0")
                expect(response.body).to.have.property('serverStatus').to.equal(2)
                expect(response.body).to.have.property('warningStatus').to.equal(0)
            ...
            cy.request({
                ...
                expect(response.body).to.have.property('affectedRows').to.equal(1)
                expect(response.body).to.have.property('insertId').to.equal(0)
                expect(response.body).to.have.property('info').to.equal("Rows matched: 1  Changed: 0  Warnings: 0")
                expect(response.body).to.have.property('serverStatus').to.equal(2)
                expect(response.body).to.have.property('warningStatus').to.equal(0)
                expect(response.body).to.have.property('changedRows').to.equal(0)
            })
        ...
        it(`POST ${adminReviewUpdatePath}/1 should return Insert Was successful`,()=>{
            ...
            cy.request({
                ...
                expect(currentSegmentUpdate).to.have.property('Segmentti').to.equal(body.Segmentti)
                expect(currentSegmentUpdate).to.have.property('Aika').to.be.a('string')
                const aika = new Date(currentSegmentUpdate.Aika);
                expect(aika instanceof Date).to.be.true;
                expect(currentSegmentUpdate).to.have.property('Kuvaus').to.equal(body.Kuvaus)
                expect(currentSegmentUpdate).to.have.property('Lumilaatu_ID1').to.equal(body.Lumilaatu_ID1)
                expect(currentSegmentUpdate).to.have.property('Lumilaatu_ID2').to.equal(body.Lumilaatu_ID2)
                expect(currentSegmentUpdate).to.have.property('Toissijainen_ID1').to.equal(body.Toissijainen_ID1)
                expect(currentSegmentUpdate).to.have.property('Toissijainen_ID2').to.equal(body.Toissijainen_ID2)
                expect(currentSegmentUpdate).to.have.property('A1_Aika').to.be.null
                expect(currentSegmentUpdate).to.have.property('A1_Lumilaatu').to.be.null
                expect(currentSegmentUpdate).to.have.property('A1_Lisätiedot').to.be.null
                expect(currentSegmentUpdate).to.have.property('A2_Aika').to.be.null
                expect(currentSegmentUpdate).to.have.property('A2_Lumilaatu').to.be.null
                expect(currentSegmentUpdate).to.have.property('A2_Lisätiedot').to.be.null
                expect(currentSegmentUpdate).to.have.property('A3_Aika').to.be.null
                expect(currentSegmentUpdate).to.have.property('A3_Lumilaatu').to.be.null
                expect(currentSegmentUpdate).to.have.property('A3_Lisätiedot').to.be.null
            })
        })

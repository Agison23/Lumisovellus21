...
import {
  ...
  getThreeDaysHighest,
  getThreeDaysLowest,
  getSnowDepthStatistics,
  getCurrentAirPressureInfo,
  getWinterTemperatures,
  getWinterWindStats,
} from "../weather/DataCalculations";
...
function WeatherTab() {
  ...
  const [displayWeatherStatistics, setDisplayWeatherStatistics] =
  ...
  const fetchWeather = async () => {
    ...
    weather.dates.thirdDay = `${currentDate.getDate()}.${
    ...
    weather.dates.firstDay = `${firstDayStart.getDate()}.${
      firstDayStart.getMonth() + 1
    }.`;

    var secondDay = new Date(currentDate.getTime());
    secondDay.setDate(secondDay.getDate() - 1);

    weather.dates.secondDay = `${secondDay.getDate()}.${
          ...
          switch (result.firstElementChild.getAttribute("gml:id")) {
            ...
            case "obs-obs-1-1-t2m":
              weather.temperature = {
                ...weather.temperature,
                current:
                  result.firstElementChild.lastElementChild.lastElementChild
            ...
            case "obs-obs-1-1-p_sea":
              weather.airpressure = {
                ...weather.airpressure,
                ...getCurrentAirPressureInfo(result),
              };
    ...
    fetch(
      `https://opendata.fmi.fi/wfs/fin?service=WFS&version=2.0.0&request=GetFeature&starttime=${firstDayStart.toISOString()}&storedquery_id=fmi::observations::weather::hourly::timevaluepair&fmisid=101982&`
    )
      .then((response) => response.text())
      .then((response) => {
        const parser = new DOMParser();
        ...
        for (let result of results) {
          switch (result.firstElementChild.getAttribute("gml:id")) {
            ...
            case "obs-obs-1-1-WD_PT1H_AVG":
              weather.winddirection = {
                ...weather.winddirection,
                ...getThreeDayWindStatistics(result),
              };
              setWeatherState({ ...weatherState, ...weather.winddirection });
              break;
        ...
        if (weather.temperature.firstDayAverage >= 0) {
          ++thawDays;
          weather.temperature.thawDays.push(
            `${firstDayStart.getDate()}.${
              firstDayStart.getMonth() + 1
            }.  ${weather.temperature.firstDayAverage.toFixed()}\xB0C`
          );
        }
        if (weather.temperature.secondDayAverage >= 0) {
          ...
          weather.temperature.thawDays.push(
            `${secondDay.getDate()}.${
              secondDay.getMonth() + 1
            }.  ${weather.temperature.secondDayAverage.toFixed()}\xB0C`
          );
        }
        if (weather.temperature.thirdDayAverage >= 0) {
          ...
          weather.temperature.thawDays.push(
            `${currentDate.getDate()}.${
              currentDate.getMonth() + 1
            }.  ${weather.temperature.thirdDayAverage.toFixed()}\xB0C`
          );
        }

        weather.temperature = {
          ...weather.temperature,
          thawDaysOutOfThree: thawDays,
        };
        setWeatherState({ ...weatherState, ...weather.temperature });
      })
      .catch((error) => {
    ...
    fetch(
      `https://opendata.fmi.fi/wfs/fin?service=WFS&version=2.0.0&request=GetFeature&starttime=${snowDataStart.toISOString()}&storedquery_id=fmi::observations::weather::timevaluepair&fmisid=101987&`
    )
      .then((response) => response.text())
      .then((response) => {
        const parser = new DOMParser();
        ...
        for (let result of results) {
          switch (result.firstElementChild.getAttribute("gml:id")) {
            ...
            case "obs-obs-1-1-snow_aws":
              weather.snowdepth = {
                ...weather.snowdepth,
                ...getSnowDepthStatistics(result, currentDate),
              };
    ...
    if (winterSeason) {
      ...
      fetch(
        `https://opendata.fmi.fi/wfs/fin?service=WFS&version=2.0.0&request=GetFeature&starttime=${decemberStart.toISOString()}&storedquery_id=fmi::observations::weather::daily::timevaluepair&fmisid=101982&`
      )
        .then((response) => response.text())
        .then((response) => {
          const parser = new DOMParser();
      ...
      fetch(
        `https://opendata.fmi.fi/wfs/fin?service=WFS&version=2.0.0&request=GetFeature&starttime=${decemberStart.toISOString()}&endtime=${decemberEnd.toISOString()}&storedquery_id=fmi::observations::weather::hourly::timevaluepair&fmisid=101982&`
      )
        .then((response) => response.text())
        .then((response) => {
          const parser = new DOMParser();
          ...
          weather.december = {
            ...weather.december,
            ...getWinterWindStats(windspeeds, winddirections),
          };
          ...
          weather.winter = {
            ...weather.winter,
            maxWind: maxWind,
            strongWindDirectionX: (weather.winter.strongWindDirectionX +=
              weather.december.strongWindDirectionX),
            strongWindDirectionY: (weather.winter.strongWindDirectionY +=
              weather.december.strongWindDirectionY),
            strongWindDays: (weather.winter.strongWindDays +=
              weather.december.strongWindDays),
      ...
      if (currentMonth >= 0 && currentMonth !== 11) {
        ...
        fetch(
          `https://opendata.fmi.fi/wfs/fin?service=WFS&version=2.0.0&request=GetFeature&starttime=${januaryStart.toISOString()}&endtime=${januaryEnd.toISOString()}&storedquery_id=fmi::observations::weather::hourly::timevaluepair&fmisid=101982&`
        )
          .then((response) => response.text())
          .then((response) => {
            const parser = new DOMParser();
            ...
            weather.winter = {
              ...
              maxWind: maxWind,
              strongWindDirectionX: (weather.winter.strongWindDirectionX +=
                weather.january.strongWindDirectionX),
              strongWindDirectionY: (weather.winter.strongWindDirectionY +=
                weather.january.strongWindDirectionY),
              strongWindDays: (weather.winter.strongWindDays +=
                weather.january.strongWindDays),
              months: ++weather.winter.months,
            };
            setWeatherState({ ...weatherState, ...weather.winter });
          })
      ...
      if (currentMonth > 0 && currentMonth !== 11) {
        ...
        fetch(
          `https://opendata.fmi.fi/wfs/fin?service=WFS&version=2.0.0&request=GetFeature&starttime=${februaryStart.toISOString()}&endtime=${februaryEnd.toISOString()}&storedquery_id=fmi::observations::weather::hourly::timevaluepair&fmisid=101982&`
        )
          .then((response) => response.text())
          .then((response) => {
            const parser = new DOMParser();
            ...
            weather.winter = {
              ...
              maxWind: maxWind,
              strongWindDirectionX: (weather.winter.strongWindDirectionX +=
                weather.february.strongWindDirectionX),
              strongWindDirectionY: (weather.winter.strongWindDirectionY +=
                weather.february.strongWindDirectionY),
              strongWindDays: (weather.winter.strongWindDays +=
                weather.february.strongWindDays),
              months: ++weather.winter.months,
            };
            setWeatherState({ ...weatherState, ...weather.winter });
          })
      ...
      if (currentMonth > 1 && currentMonth !== 11) {
        ...
        fetch(
          `https://opendata.fmi.fi/wfs/fin?service=WFS&version=2.0.0&request=GetFeature&starttime=${marchStart.toISOString()}&endtime=${marchEnd.toISOString()}&storedquery_id=fmi::observations::weather::hourly::timevaluepair&fmisid=101982&`
        )
          .then((response) => response.text())
          .then((response) => {
            const parser = new DOMParser();
            ...
            weather.winter = {
              ...
              maxWind: maxWind,
              strongWindDirectionX: (weather.winter.strongWindDirectionX +=
                weather.march.strongWindDirectionX),
              strongWindDirectionY: (weather.winter.strongWindDirectionY +=
                weather.march.strongWindDirectionY),
              strongWindDays: (weather.winter.strongWindDays +=
                weather.march.strongWindDays),
              months: ++weather.winter.months,
            };
            setWeatherState({ ...weatherState, ...weather.winter });
          })
      ...
      if (currentMonth > 2 && currentMonth !== 11) {
        ...
        fetch(
          `https://opendata.fmi.fi/wfs/fin?service=WFS&version=2.0.0&request=GetFeature&starttime=${aprilStart.toISOString()}&endtime=${aprilEnd.toISOString()}&storedquery_id=fmi::observations::weather::hourly::timevaluepair&fmisid=101982&`
        )
          .then((response) => response.text())
          .then((response) => {
            const parser = new DOMParser();
            ...
            weather.winter = {
              ...
              maxWind: maxWind,
              strongWindDirectionX: (weather.winter.strongWindDirectionX +=
                weather.april.strongWindDirectionX),
              strongWindDirectionY: (weather.winter.strongWindDirectionY +=
                weather.april.strongWindDirectionY),
              strongWindDays: (weather.winter.strongWindDays +=
                weather.april.strongWindDays),
              months: ++weather.winter.months,
            };
            setWeatherState({ ...weatherState, ...weather.winter });
          })
      ...
      if (currentMonth > 3 && currentMonth !== 11) {
        ...
        fetch(
          `https://opendata.fmi.fi/wfs/fin?service=WFS&version=2.0.0&request=GetFeature&starttime=${mayStart.toISOString()}&endtime=${mayEnd.toISOString()}&storedquery_id=fmi::observations::weather::hourly::timevaluepair&fmisid=101982&`
        )
          .then((response) => response.text())
          .then((response) => {
            const parser = new DOMParser();
            ...
            weather.winter = {
              ...
              maxWind: maxWind,
              strongWindDirectionX: (weather.winter.strongWindDirectionX +=
                weather.may.strongWindDirectionX),
              strongWindDirectionY: (weather.winter.strongWindDirectionY +=
                weather.may.strongWindDirectionY),
              strongWindDays: (weather.winter.strongWindDays +=
                weather.may.strongWindDays),
              months: ++weather.winter.months,
            };
            setWeatherState({ ...weatherState, ...weather.winter });
          })
